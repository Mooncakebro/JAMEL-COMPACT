...
class SokobanEnvironmentManager(EnvironmentManagerBase):
    ...
    def step(self, text_actions: List[str]):
        ...
        if self.is_multi_modal:
            self.memory.store(
                {
                    'text_obs': [None for _ in actions],
                    'image_obs': [obs for obs in self.pre_image_obs],
...
class JAMELEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.memory = SimpleMemory()
        self.current_prompt_texts = []
        super().__init__(envs, projection_f, config)

    @staticmethod
    def _update_trace_row(trace_path: str, step: int, updates: dict[str, Any]) -> None:
        ...
        updated = False
        for row in rows:
            if int(row.get("step", -1) or -1) == int(step):
                row.update(updates)
                updated = True
                break
        if not updated:
        ...
        pq.write_table(pa.Table.from_pylist(normalized_rows), path)

    def reset(self, kwargs) -> Dict[str, Any]:
        raw_obs, infos = self.envs.reset()
        text_obs = [obs["text"] if isinstance(obs, dict) else obs for obs in raw_obs]
        image_obs = [
            obs.get("image") if isinstance(obs, dict) else None
            for obs in raw_obs
        ]
        self.tasks = [info.get("goal", "Explore the website to maximize novel coverage.") for info in infos]
        self.target_urls = [info.get("target_url", "") for info in infos]
        self.pre_text_obs = text_obs
        self.pre_image_obs = image_obs
        self.memory.reset(batch_size=len(infos))

        prompt_texts = self.build_text_obs(text_obs, infos, init=True)
        self.current_prompt_texts = prompt_texts
        observations = {
            "text": prompt_texts,
            "image": image_obs if any(image is not None for image in image_obs) else None,
            "anchor": text_obs.copy(),
        }
        for i, info in enumerate(infos):
            trace_path = info.get("trace_path")
            if trace_path:
                self._update_trace_row(
                    trace_path,
                    step=0,
                    updates={
    ...
    def step(self, text_actions: List[str]):
        ...
        next_obs = [obs["text"] if isinstance(obs, dict) else obs for obs in raw_next_obs]
        next_image_obs = [
            obs.get("image") if isinstance(obs, dict) else None
            for obs in raw_next_obs
        ]

        self.memory.store({
            "text_obs": self.pre_text_obs,
            "image_obs": self.pre_image_obs,
            "action": actions,
        })
        self.pre_text_obs = next_obs
        self.pre_image_obs = next_image_obs

        next_prompt_texts = self.build_text_obs(next_obs, infos)
        next_observations = {
            "text": next_prompt_texts,
            "image": next_image_obs if any(image is not None for image in next_image_obs) else None,
            "anchor": next_obs.copy(),
        }

        for i, info in enumerate(infos):
            info["is_action_valid"] = to_numpy(valids[i])
            info["raw_response"] = text_actions[i]
            info["projected_action"] = actions[i]
            info["prompt_text"] = self.current_prompt_texts[i] if i < len(self.current_prompt_texts) else None
            trace_path = info.get("trace_path")
            episode_step = info.get("episode_step")
            if trace_path and episode_step is not None:
                self._update_trace_row(
                    trace_path,
                    step=int(episode_step),
                    updates={
                        "prompt_text": info["prompt_text"],
                        "raw_response": text_actions[i],
                        "projected_action": actions[i],
                    },
                )

        self.current_prompt_texts = next_prompt_texts
    ...
    def build_text_obs(self, text_obs: List[str], infos: List[Dict[str, Any]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            current_observation = text_obs[i]
            if init or self.config.env.history_length <= 0:
                obs = JAMEL_TEMPLATE_NO_HIS.format(
                    task_description=self.tasks[i],
                    target_url=self.target_urls[i],
                    action_space=JAMEL_ACTION_SPACE,
                    current_observation=current_observation,
                )
            else:
                obs = JAMEL_TEMPLATE.format(
                    task_description=self.tasks[i],
                    target_url=self.target_urls[i],
                    step_count=len(self.memory[i]),
                    history_length=valid_lens[i],
    ...
    def success_evaluator(self, *args, **kwargs) -> Dict[str, np.ndarray]:
        episode_rewards = np.asarray(kwargs["episode_rewards"], dtype=np.float32)
        episode_lengths = np.asarray(kwargs["episode_lengths"], dtype=np.float32)
        safe_lengths = np.maximum(episode_lengths, 1.0)
        return {
            "success_rate": (episode_rewards > 0).astype(np.float32),
            "coverage_reward": episode_rewards,
            "coverage_reward_per_step": episode_rewards / safe_lengths,
        }
...
def make_envs(config):
    ...
    if not isinstance(config.env.rollout.n, int):
        config.env.rollout.n = int(config.env.rollout.n)
        print(f"config.env.rollout.n: {type(config.env.rollout.n)}, {config.env.rollout.n}")
    ...
    if "search" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "jamel" in config.env.env_name.lower():
        from agent_system.environments.env_package.jamel import (
            build_jamel_envs,
            jamel_projection,
        )

        env_kwargs = {
            "target_urls": list(config.env.jamel.target_urls),
            "goal": config.env.jamel.goal,
            "env_id": config.env.jamel.env_id,
            "headless": config.env.jamel.headless,
            "timeout": config.env.jamel.timeout,
            "worker_timeout": config.env.jamel.worker_timeout,
            "record_coverage": config.env.jamel.record_coverage,
            "coverage_dir": config.env.jamel.coverage_dir
            or os.path.join(
                os.getcwd(),
                "outputs",
            ...
            "trace_mode": config.env.jamel.trace_mode,
            "trace_freq": config.env.jamel.trace_freq,
            "max_traces_per_dump": config.env.jamel.max_traces_per_dump,
            "trace_image": config.env.jamel.trace_image,
            "frozen_coverage_paths": list(config.env.jamel.frozen_coverage_paths),
            "frozen_coverage_manifest": config.env.jamel.frozen_coverage_manifest,
            "jamel_root": OmegaConf.select(config, "env.jamel.jamel_root", default=None),
        }

        _envs = build_jamel_envs(
        ...
        projection_f = partial(jamel_projection)
        envs = JAMELEnvironmentManager(_envs, projection_f, config)
        val_envs = JAMELEnvironmentManager(_val_envs, projection_f, config)
        return envs, val_envs
