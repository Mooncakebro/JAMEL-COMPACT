...
- [6. JAMEL Browser Environment](#6-jamel-browser-environment)
...
---
## 6. JAMEL Browser Environment
This environment uses `browsergym/openended` plus a step-level coverage reward computed from raw Chrome precise coverage snapshots.

Install the browser dependencies:
```bash
cd repo_root/
pip install browsergym==0.14.0 "playwright>=1.40.0" gymnasium==0.29.1
playwright install chromium
pip install -e .
```

At training time, point `env.jamel.target_urls` to one or more local websites. A minimal GRPO launcher is available at:
```bash
examples/grpo_trainer/run_jamel.sh
```

The launcher expects your site to already be running, for example:
```bash
TARGET_URL=http://localhost:8000/weibo/ bash examples/grpo_trainer/run_jamel.sh
```
