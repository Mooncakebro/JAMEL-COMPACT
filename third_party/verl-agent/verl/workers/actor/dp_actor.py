...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        memory_inputs = {}
        if "memory_tokens" in micro_batch:
            memory_inputs["memory_tokens"] = micro_batch["memory_tokens"]
        if "memory_attention_mask" in micro_batch:
            memory_inputs["memory_attention_mask"] = micro_batch["memory_attention_mask"]
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        if "memory_tokens" in data.batch.keys():
            select_keys.append("memory_tokens")
        if "memory_attention_mask" in data.batch.keys():
        ...
        _profile_log(
            ...
            has_memory="memory_tokens" in data.batch.keys(),
        )
    ...
    def update_policy(self, data: DataProto):
        ...
        profile_batch_size = data.batch.batch_size[0]
        profile_has_memory = "memory_tokens" in data.batch.keys()
            ...
            for batch_idx, mini_batch_data in enumerate(dataloader):
                ...
                for micro_batch in micro_batches:
                    ...
                    if isinstance(micro_batch, DataProto):
                        ...
                        micro_batch = micro_batch.to(get_torch_device().current_device())  # actor device is cpu when using offload
