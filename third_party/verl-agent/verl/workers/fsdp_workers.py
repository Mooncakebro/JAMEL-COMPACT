...
def _apply_lora_to_custom_model(model: torch.nn.Module, config) -> torch.nn.Module:
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if custom_model_enabled:
                if use_remove_padding or self.ulysses_sequence_parallel_size > 1:
                    raise ValueError("Custom memory-augmented actor/ref models require use_remove_padding=False and ulysses_sequence_parallel_size=1.")
                actor_module = instantiate_custom_model(
            ...
            if use_liger and not custom_model_enabled:
            ...
            if self._is_lora:
                ...
                if custom_model_enabled:
                    actor_module = _apply_lora_to_custom_model(actor_module, self.config.model)
                else:
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            rollout = HFRollout(
                ...
                memory_config=self.config.model.get("memory_augment", None),
            )
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        custom_model_enabled = has_custom_model(config.model.get("custom_cls", None))
        with init_context(), warnings.catch_warnings():
            warnings.simplefilter("ignore")
            if custom_model_enabled:
                if config.model.get("use_remove_padding", False) or self.ulysses_sequence_parallel_size > 1:
                    raise ValueError("Custom memory-augmented critic models require use_remove_padding=False and ulysses_sequence_parallel_size=1.")
                critic_module = instantiate_custom_model(
                    custom_cfg=config.model.custom_cls,
                    pretrained_model_name_or_path=local_path,
                    ...
                    extra_kwargs={
                        "memory_augment_config": resolve_memory_augment_config(
                            config.model.get("memory_augment", None)
                        )
                    },
                )
            else:
        ...
        if self._is_lora:
            ...
            if custom_model_enabled:
                critic_module = _apply_lora_to_custom_model(critic_module, self.config.model)
            else:
