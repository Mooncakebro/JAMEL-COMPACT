...
summarize_str = lambda max_char_count: lambda x: x[:max_char_count] + f"\n...({len(x) - max_char_count * 2} more chars)\n" + x[-max_char_count:] if len(x) > max_char_count * 2 else x
...
def make_user_serialized(*args):
    return make_user(*args).serialize()

def make_assistant_serialized(*args):
    return make_assistant(*args).serialize()

def make_system_serialized(*args):
    return make_system(*args).serialize()


def make_user_messages(*args):
    return [make_user(*args).serialize()]

def make_assistant_messages(*args):
    return [make_assistant(*args).serialize()]

def make_system_messages(*args):
    return [make_system(*args).serialize()]
...
def log_retry_error(retry_state):
    retry_logger.warning(f"Function failed after retries: {retry_state.outcome.exception()}")
